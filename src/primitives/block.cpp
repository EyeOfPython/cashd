...
std::string CBlock::ToString() const {
    std::stringstream s;
    s << strprintf("CBlock(hash=%s, hashPrevBlock=%s, bits=0x%08x, time=%u, "
                   "nonce=%u, headerversion=%u, size=%u, height=%d, hashEpochBlock=%s, "
                   "hashMerkleRoot=%s, hashExtendedMetadata=%s, vtx=%u)\n",
                   GetHash().ToString(), hashPrevBlock.ToString(), nBits,
                   GetBlockTime(), nNonce, nHeaderVersion, GetSize(), nHeight,
                   hashEpochBlock.ToString(), hashMerkleRoot.ToString(),
                   hashExtendedMetadata.ToString(), vtx.size());
